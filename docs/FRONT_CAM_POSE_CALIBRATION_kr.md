...
2. 쌍을 사용 가능 화면 가운데로 되돌린 뒤 **폐루프 피벗** 8회: 태그로 잰
   실제 회전이 첫 스냅샷 기준 누적 +F/2, +F, +F/2, 0, −F/2, −F, −F/2, 0
   (±0.5°)이 될 때까지 명령·측정·게인 학습을 반복한다 (베이스는 2° 명령을
   0.3~0.8°만, 5° 명령을 ~3.7° 수행한다 — 2026-09-15 실측; 명령량은
   루프가 알아서 키운다). **F는 화면이 허락하는 최대 회전**으로 현장에서
   계산한다 (`pivot targets … deg` 로그): 세로 0.24 m 화면에서 태그가
   0.6 m 레버로 옆으로 쓸리므로 두 장을 다 남기면 **약 ±6.5°가 물리
   한계**다 (지금 배치: F ≈ 6.4°, 스팬 ≈ 13°). `--pivot-one-tag`를 주면
   끝점에서 먼 태그가 화면을 벗어나는 대신 ±8°까지 간다(그 스냅샷은
   한 장짜리로 쓴다). `--pivot-max`는 상한만 낮춘다. 피벗 속도는
   `--pivot-speed` 0.08 rad/s (mobile_node 기본 0.2는 너무 빨랐음).
   단계 시작 때 태그가 화면 세로 중앙에 오도록 스스로 되돌리므로, 이전
   스윕이 끝에서 멈춘 채로 다시 시작해도 된다. `mobile_node`가 오도메트리
   기준 "부족하게 돌았다"고 실패를 보고해도 태그가 움직였으면 정상 진행
   한다. 각각 뒤 스냅샷 → `piv_NN_*.txt` (9장).
